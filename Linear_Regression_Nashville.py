...
def load_data(spark, inputs):
    """
    Loads Business, Checkin, and Listings data.
    """
    ...
    print(f"Loading data from {inputs} and {airbnb_base_path}...")

    return (
        spark.read.parquet(business_path),
        spark.read.parquet(checkin_path),
        spark.read.csv(listings_path, header=True, inferSchema=True, multiLine=True, escape='"')
    )
...
def train_model(master_df):
    """
    Trains Linear Regression to find the 'Trend Line' of revenue.
    """
    ...
    lr = LinearRegression(featuresCol="features", labelCol="avg_actual_revenue", regParam=0.1)
    ...
    return lr_model.transform(ml_data)
...
if __name__ == '__main__':
    ...
    output = os.getenv("S3_OUTPUT_MODEL_PATH", "/home/ubuntu/nashville_investment_map_output")
    
    spark = SparkSession.builder \
        .appName('Nashville Linear Regression Analysis') \
